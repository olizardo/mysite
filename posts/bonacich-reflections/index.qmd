...
Let's see how that looks like with real data. The results of the reflection equations for the Southern Women data, for people up to sixteen reflections, are shown in @tbl-refp. The corresponding results for groups are shown in @tbl-refg. 
...
Which means that $c_p$ is an eigenvector of the projection matrix $B_p$ and $c_g$ is an eigenvector of the projection matrix $B_g$. In fact, the vector of centrality scores that will order people exactly as in @tbl-final, are the leading (first) eigenvectors of each projection matrix (Bonacich, 1991; Faust, 1997). These are shown in @tbl-eigen. 
