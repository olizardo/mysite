...
```{r}
    ...
    p <- paths[which(sapply(paths, length) == 4)]
...
<br>
```{r}
    paths <- all_simple_paths(gr, from = toupper(letters[1:length(d)]), cutoff = 2)
    p <- paths[which(sapply(paths, length) == 3)]
    m <- toupper(matrix(letters[unlist(p)], ncol = 3, byrow = TRUE))
    m <- m[which(m[, 2] == "B"), ]
    kbl(m)
